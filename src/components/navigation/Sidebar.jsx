...
function Sidebar() {
    ...
    return (
        <aside className="w-16 h-screen flex flex-col items-center">
            <div className="mb-6" onClick={navigateToHome}>
                <svg
                    aria-label="Threads"
                    className="mt-3 size-8 cursor-pointer transition-all hover:scale-110 fill-neutral-900 dark:fill-neutral-100"
            ...
            <nav className="flex-1 flex flex-col justify-center gap-4">
                {NAV_ITEMS.map((item) => {
                    ...
                    return (
                        <div // Dùng div hoặc button thay vì NavLink trực tiếp để kiểm soát click tốt hơn
                            key={path}
                            onClick={() => {
